...
OUTPUT_ACTION_FILTERED = "../../dataset/filtered_data/action_channel_filtered_dataset.csv"
OUTPUT_REPORT = "filter_action_channels_report.txt"
...
whitelist_norm = {
    x.strip().lower()
    for x in ACTION_CHANNEL_WHITELIST
}
...
filtered_df = df[
    df["actionNorm"].isin(whitelist_norm)
].copy()
...
matched_channels = (
    set(df["actionNorm"].dropna().unique())
    & whitelist_norm
)
